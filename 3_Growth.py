...
with st.sidebar:
    ...
    if d:
        ...
        st.markdown('<span style="font-size:0.72rem;padding:2px 8px;border-radius:4px;background:#3b2a0f;color:#fbbf24">⚠ Enter ticker and fetch</span>', unsafe_allow_html=True)
    st.markdown("---")
...
if not d:
    st.info("Enter a ticker in the sidebar and click **🔄 Fetch live data**.")
    st.stop()
